  ...
  $.each(result, function (ind, item) {
    if(type === 'category') {
      ...
      $('#card' + type + 'menu').append('<div class="ui item" data-value="' + item.id + '">' +
        '<small>' + item.date + '</small>  ' + item.value + '  (' + item.valid + ')   ~<small>' + Math.round(item.counter/1000) + 'k cards</small></div>');
    }
    if(type === 'country') {
...
function change_firts_settings() {
  $('#categoryblock,#countryblock,#brandblock,#typeblock,#levelblock,#stateblock,#cityblock').dropdown('clear', 'restore defaults');
  $('#cardcategorymenu *,#cardcountrymenu *,#cardbrandmenu *,#cardtypemenu *,#cardlevelmenu *,#cardstatemenu *,#cardcitymenu *').remove();
  get_list('cardcategorymenu', 'All', 'category');
}

function change_category() {
  $('#countryblock,#brandblock,#typeblock,#levelblock,#stateblock,#cityblock').dropdown('clear', 'restore defaults');
  $('#cardbrandmenu *,#cardtypemenu *,#cardlevelmenu *,#cardstatemenu *,#cardcitymenu *').remove();
  get_list('cardcountrymenu', 'All', 'country');
}

function change_country() {
  $('#brandblock,#typeblock,#levelblock,#stateblock,#cityblock').dropdown('clear', 'restore defaults');
  $('#cardbrandmenu *,#cardtypemenu *,#cardlevelmenu *,#cardstatemenu *,#cardcitymenu *').remove();

  if($('#cardcountry').val() !== '0' && $('#cardcountry').val() !== '') {
    get_list('cardbrandmenu', 'All', 'brand');
    get_list('cardstatemenu', 'All', 'state');
  }
}

function change_brand() {
  $('#typeblock,#levelblock,#stateblock,#cityblock').dropdown('clear', 'restore defaults');
  $('#cardtypemenu *,#cardlevelmenu *,#cardstatemenu *,#cardcitymenu *').remove();
  if($('#cardbrand').val() !== '') {
    get_list('cardtypemenu', 'All', 'type');
    get_list('cardstatemenu', 'All', 'state');
  }
}

function change_type() {
  $('#levelblock,#stateblock,#cityblock').dropdown('clear', 'restore defaults');
  $('#cardlevelmenu *,#cardstatemenu *,#cardcitymenu *').remove();
  if($('#cardtype').val() !== '') {
    get_list('cardlevelmenu', 'All', 'level');
    get_list('cardstatemenu', 'All', 'state');
  }
}
...
function add_card_line(item) {
  ...
  $('#cardbody').append('' +
    ...
    '<td>' + item.month + ' / ' + item.year + '<br>' +
    '<small>CVV: ' +
    add_symbol(item.excvv) +
    '<br>' +
    '<small>Name: ' + item.fname + '</small></td>\n' +
    '<td>' + item.country + '<br><small><strong>' + item.bank + '</strong><br>Base: ' + item.catdate + ' ' + item.catname + '</small></td>\n' +
    '<td><small>Address: ' + add_symbol(item.address) + '<br>Phone: ' + add_symbol(item.phone) + '<br>eMail: ' + add_symbol(item.email) + '</small></td>\n' +
    '<td>' + item.city + '</td>\n' +
    '<td>' + item.state + '</td>\n' +
    '<td>' + item.zip + '</td>\n' +
    '<td>$' + item.price + '<br><small>Refund: ' + add_symbol(item.nocheckcc) + '</small></td>\n' +
    '<td>' +
  ...
  $.ajax({
    ...
    success: function (item) {
      if(item.details !== undefined) {
        ...
        $('#pay_data').html(
          '<tr>\n' +
          '        <td class="right aligned">' + item.type + ' by course:</td>\n' +
          '        <td>$' + course + '</td>\n' +
          '      </tr>' +
          '<tr>\n' +
        ...
        //setTimeout(function (){
        //  document.location="/money_add.php";
        // },2000);
      }
    },
    complete: function () {
